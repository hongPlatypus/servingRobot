...
class TableServer(Node):
    ...
    async def handle_soldout(self, request, response):
        ...
        try:
            if self._future is not None and not self._future.done():
                ...
                response.message = "Another request is already being processed."
                return response
            ...
            response.message = "Handled sold-out items."
        except Exception as e:
            self.get_logger().error(f"Error in handling /soldout: {e}")
            response.success = False
            response.message = "Failed to handle sold-out notification."
    ...
    def handle_soldout_approve(self, request, response):
        ...
        response.success = True
        response.message = "Sold-out approval acknowledged"
        return response
