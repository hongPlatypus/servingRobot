...
class DBHandler:
    ...
    def get_current_stock(self, item_name=None):
        """
        현재 재고량 확인. 특정 항목의 재고를 가져오거나, 전체 항목의 재고를 반환합니다.
        """
        ...
        # 전체 재고 반환
        try:
            query = "SELECT jokbal_mid_stock, jokbal_lar_stock, jinro_stock, cham_stock FROM STOCK WHERE stock_date = CURRENT_DATE"
            self.cursor.execute(query)
            ...
            print(f"Error retrieving all stock: {err}")
            return {}
    ...
    def insert_order(self, table_id, items):
        ...
        try:
            ...
            # STOCK 테이블 업데이트 (재고 차감)
            stock_update_query = """
            UPDATE STOCK
            SET 
                jokbal_mid_stock = jokbal_mid_stock - %s,
                jokbal_lar_stock = jokbal_lar_stock - %s,
                jinro_stock = jinro_stock - %s,
                cham_stock = cham_stock - %s
            WHERE stock_date = %s
            """
            self.cursor.execute(stock_update_query, (
                items.get("족발 중", 0), items.get("족발 대", 0),
                items.get("진로", 0), items.get("참이슬", 0),
                current_date
            ))
            print(f"✅ STOCK 테이블 업데이트 결과: {self.cursor.rowcount} rows affected")
            
            if self.cursor.rowcount == 0:
                print("❌ STOCK 업데이트 실패: 해당 날짜의 레코드가 없습니다.")
            self.conn.commit()
    ...
    def get_all_stock(self):
        """
        모든 제품의 현재 재고를 반환.
        :return: dict {item_name: stock_count}
        """
        query = "SELECT item_name, stock FROM inventory;"
        try:
            self.cursor.execute(query)
            results = self.cursor.fetchall()
            return {row['item_name']: row['stock'] for row in results}
    ...
    def process_order(self, items):
        ...
        try:
            ...
            for item_name, quantity in items.items():
                ...
                stock = self.db_handler.cursor.fetchone()
                stock_value = stock[0] if stock else 0
                print(f"Item: {item_name}, Current Stock: {stock_value}, Quantity Ordered: {quantity}")
                ...
                if stock_value < quantity:
                    ...
                    WHERE stock_date = CURRENT_DATE
                    """
                    print(f"Executing Update Query: {update_query} with Quantity: {quantity}")
                    self.db_handler.cursor.execute(update_query, (quantity,))
